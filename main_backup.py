...
class Model(object):
    ...
    def load_world(self):
        try:
            ...
            self.water_height = random.randrange(6,8)
    ...
    def generate_sector(self, mapSector, sector, show=True):
        ...
        for x in xrange(SECTOR_SIZE):
                for z in xrange(SECTOR_SIZE):
                    ...
                    self.add_block((posx, int(terrainHeight) + 5, posz), SAND, immediate=False)
                    ...
                    if terrainHeight == self.water_height: # Add water
                        self.add_block((posx, int(terrainHeight) + y + 5, posz), WATER, immediate=False)
        ...
        if terrainHeight+6 > self.water_height and len(surface) > 5:
...
class Window(pyglet.window.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.position = ((maxSector-mapSize/2) * SECTOR_SIZE, 100, (maxSector-mapSize/2) * SECTOR_SIZE)
        ...
        # TODO uncomment this
        self.model.generate_sectors(self.sector, self.sector,50, False)
    ...
    def _update(self, dt):
        ...
        if x < SECTOR_SIZE * (maxSector - mapSize):
            x = maxSector * SECTOR_SIZE
        if z < SECTOR_SIZE * (maxSector - mapSize):
            z = maxSector * SECTOR_SIZE
        if x > maxSector * SECTOR_SIZE:
            x =  SECTOR_SIZE * (maxSector - mapSize)
        if z > maxSector * SECTOR_SIZE:
            z =  SECTOR_SIZE * (maxSector - mapSize)
        # collisions
