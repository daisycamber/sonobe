...
view_distance = mapSize+2 #12

LOAD_SECTOR_DIST = view_distance
...
START_POS = ((maxSector - mapSize/2) * SECTOR_SIZE,100,(maxSector - mapSize/2) * SECTOR_SIZE)
...
fog = (0.5,0.69,1)
# Quick and dirty, fix this to be updated later on when the player enters a new planet
if planet_type == 3:
    fog = (212/255, 85/255, 68/255)
if planet_type == 2:
    fog = (1, 1, 1)
if planet_type == 1:
    fog = (126/255, 0, 204/255)

sky = (0.5,0.69,1)
# Quick and dirty, fix this to be updated later on when the player enters a new planet
if planet_type == 3:
    sky = (212/255, 85/255, 68/255)
if planet_type == 2:
    sky = (0.1, 0.1, 0.1)
if planet_type == 1:
    sky = (126/255, 0, 204/255)

BLOCKS = 14
GRASS = 0 + BLOCKS * planet_type
SAND = 1 + BLOCKS * planet_type
BRICK = 2 + BLOCKS  * planet_type
BEDROCK = 3 + BLOCKS * planet_type
STONE = 4 + BLOCKS * planet_type
WOOD = 5 + BLOCKS * planet_type
LEAVES = 6 + BLOCKS * planet_type
DIRT = 7 + BLOCKS * planet_type
WATER = 8 + BLOCKS * planet_type
COAL_ORE = 9 + BLOCKS * planet_type
IRON_ORE = 10 + BLOCKS * planet_type
COPPER_ORE = 11 + BLOCKS * planet_type
TIN_ORE = 12 + BLOCKS * planet_type
GOLD_ORE = 13 + BLOCKS * planet_type
...
#Alien
textures.append([111, 8, 166])
textures.append([194, 119, 76])
textures.append([179,77,77])
textures.append([50,50,50])
textures.append([90, 66, 102])
textures.append([214, 2, 204])
textures.append([0, 173, 104])
textures.append([168, 59, 29])
textures.append([105, 207, 209])
textures.append([69,69,69])
textures.append([161,65,55])
textures.append([158,49,36])
textures.append([163,163,163])
...
class Model(object):
    ...
    def load_world(self):
        try:
            ...
            self.water_height = random.randrange(10,13)
    ...
    def show_block(self, position, immediate=True):
        ...
        #t1.start() 
            ...
            #threading.Thread(target=self._enqueue, args=(self._show_block, position, texture,)).start() 
    ...
    def _show_block(self, position, texture):
        ...
        R = int(textures[texture][0] * red)
        G = int(textures[texture][1] * green)
        B = int(textures[texture][2] * blue)
        color_data = [
            R,G,B,R,G,B,R,G,B,R,G,B,  # top
    ...
    def generate_sector(self, mapSector, sector, show=True):
        ...
        crater_x = mapSector[0] * SECTOR_SIZE + random.randrange(4,SECTOR_SIZE-4)
        crater_z = mapSector[2] * SECTOR_SIZE + random.randrange(4,SECTOR_SIZE-4)
        crater_diameter = random.randrange(2,5)
        
        for x in xrange(SECTOR_SIZE):
                for z in xrange(SECTOR_SIZE):
                    # Get the height of the terrain from perlin noise
                    posx = mapSector[0] * SECTOR_SIZE + x
                    posz = mapSector[2] * SECTOR_SIZE + z
                    # 0 for earthlike, 1 for alien,  2 for moon, 3 for mars, etc
                    if planet_type == 2:
                        terrainHeight = noise.pnoise2((noiseOffsetX + posx)/scale, 
                                    (noiseOffsetY + posz)/scale, 
                                    octaves=octaves, 
                    ...
                    #self.add_block((posx, 0 - 1, posz), BEDROCK, immediate=False)
                    self.world[(posx, -1, posz)] = BEDROCK
                    for y in xrange(int(terrainHeight)): # Stone layer
                    #    self.add_block((posx, y, posz), STONE, immediate=False)

                        self.world[(posx, y, posz)] = STONE
                        # 0 for earthlike, 1 for alien,  2 for moon, 3 for mars, etc
                    if (planet_type == 0 or planet_type == 1):
                        if(terrainHeight < self.water_height):
                            self.add_block((posx, int(terrainHeight), posz), SAND, immediate=False)
                        
                        #for y in xrange(4): # Sand layer
                        #    self.add_block((posx, int(terrainHeight) + y, posz), SAND, immediate=False)
                        #    self.world[(posx, int(terrainHeight) + y, posz)] = SAND
                        self.add_block((posx, self.water_height, posz), WATER, immediate=False)
                            #for y in xrange(self.water_height - int(terrainHeight)): # Water layer
                                #
                                #if y != self.water_height - int(terrainHeight):
                                    #self.hide_block((posx, int(terrainHeight) + y + 5, posz), True)
                        
                        for y in xrange(5): # Dirt layer
                                #self.add_block((posx, int(terrainHeight) + y, posz), DIRT, immediate=False)
                            self.world[(posx, int(terrainHeight) + y, posz)] = DIRT
                                
                        # Grass layer
                        self.add_block((posx, int(terrainHeight) + 5, posz), GRASS, immediate=False)
                        if x > 0 and x < 15 and z > 0 and z < 15:
                            surface.append((posx, int(terrainHeight) + 5, posz))
                        else:
                            for minusy in range(2):
                                #if self.exposed((posx, int(terrainHeight) + 4 - minusy, posz)):
                                self.add_block((posx, int(terrainHeight) + 4 - minusy, posz), DIRT, immediate=False)
                                #self.add_block((posx, int(terrainHeight) + 5 - 2, posz), DIRT, immediate=False)
                                #self.add_block((posx, int(terrainHeight) + 5 - 3, posz), DIRT, immediate=False)
                                #self.check_neighbors((posx, int(terrainHeight) + 5, posz))
                    else:
                        
                        #if distance((crater_x,int(terrainHeight-1),crater_z),(posz,int(terrainHeight-1),posz)) >= crater_diameter/2:
                        self.add_block((posx, int(terrainHeight), posz), STONE, immediate=False)
                        #else:
                        #    print("Generating crater")
                        #    print(crater_diameter)
                        #    self.add_block((posx, int(terrainHeight-2), posz), STONE, immediate=False)
                        if not (x > 0 and x < 15 and z > 0 and z < 15):
                            for minusy in range(2):
                                #if self.exposed((posx, int(terrainHeight) + 4 - minusy, posz)):
                                self.add_block((posx, int(terrainHeight) - minusy, posz), STONE, immediate=False)
                                    #self.add_block((posx, int(terrainHeight) + 5 - 2, posz), DIRT, immediate=False)
                                    #self.add_block((posx, int(terrainHeight) + 5 - 3, posz), DIRT, immediate=False)
                                    #self.check_neighbors((posx, int(terrainHeight) + 5, posz))
                        else:
                            surface.append((posx, int(terrainHeight), posz))
                
        # Add ores
        for x in range(ORES_PER_SECTOR):
            try:
                self.add_ore((mapSector[0] * SECTOR_SIZE + random.randrange(0,SECTOR_SIZE), random.randrange(0,int(terrainHeight-3)), mapSector[2] * SECTOR_SIZE + random.randrange(0,SECTOR_SIZE)), random.randrange(ORE_MIN,ORE_MAX), random.randrange(COAL_ORE, GOLD_ORE))
            except:
                pass
        if terrainHeight+2 > self.water_height and len(surface) > 5 and planet_type == 0 or planet_type == 1:
        ...
        if planet_type == 2 or planet_type == 3:
            #print("Generating crater")
            crater_pos = surface[random.randrange(0,len(surface))]
            crater_x = crater_pos[0]
            crater_y = crater_pos[1]
            crater_z = crater_pos[2]
            crater_diameter = random.randrange(4,8)
            crater_radius = crater_diameter/2+1
            #print(crater_diameter)
            for x in xrange(crater_diameter):
                for z in xrange(crater_diameter):
                    pos = (math.floor(crater_x + x - crater_radius),crater_y,math.floor(crater_z + z - crater_radius))
                    if distance(pos,crater_pos) <= crater_radius:
                        #print(pos)
                        #try:
                        self.remove_block(pos,immediate=False)
                        self.add_block((pos[0],pos[1]-1,pos[2]),STONE,immediate=False)
                        #except:
    ...
    def show_sector(self, sector):
        ...
        for position in self.sectors.get(tuple(sec), []):#self.sectors.get(sector, []):
            ...
            #    self.check_neighbors(position)
            p = (int(position[0] % SECTOR_SIZE) + ((sector[0]) * SECTOR_SIZE),int(position[1]),int(position[2] % SECTOR_SIZE) + (sector[2] * SECTOR_SIZE))
    ...
    def change_sectors(self, before, after):
        ...
        for sector in hide:
            threading.Thread(target=self.hide_sector, args=(sector,)).start() 
...
class Window(pyglet.window.Window):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # TODO uncomment this
        self.model.generate_sectors(self.sector, self.sector,7, False)
    ...
    def update(self, dt):
        ...
        sector = sectorize(self.position)
        if sector != self.sector2:
            self.changed_sector = self.changed_sector + 1
            print(distance(self.sector, sector))
            if distance(self.sector, sector) >= LOAD_SECTOR_DIST:
        ...
        if self.runtime % SAVE_FREQUENCY == 0:
            print("Saving world")
    ...
    def _update(self, dt):
        ...
        if x < (mapSize - 1) * SECTOR_SIZE * maxSector:
            x = (maxSector) * SECTOR_SIZE
        if z < (mapSize - 1) * SECTOR_SIZE * maxSector:
            z = (maxSector) * SECTOR_SIZE
        if x > (maxSector) * SECTOR_SIZE:
            x = (mapSize - 1) * SECTOR_SIZE * maxSector
        if z > (maxSector) * SECTOR_SIZE:
            z = (mapSize - 1) * SECTOR_SIZE * maxSector
        # collisions
    ...
    def on_draw(self):
        ...
        glFogfv(GL_FOG_COLOR, (GLfloat * 4)(fog[0] * self.alpha, fog[1] * self.alpha, fog[2] * self.alpha, 1))
        glClearColor(sky[0], sky[1], sky[2],self.alpha)
        #glTranslatef(-self.position[0],-256,-self.position[2])
        #glPopMatrix()
    ...
    #    glColor3d(1, 1, 1)
    #    for x in range(len(self.crafting_sprites)):
